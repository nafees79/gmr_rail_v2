...
from api.service.models import RailData, rcrrakeQuota
...
class RcrRakeQuota:
    ...
    def coal_journey_rcr_rake_quota_pending(self, currentPage, perPage, start_timestamp, end_timestamp):
        try:
            pending_filters = Q()
            result = {
                "labels": [],
            ...
            try:
                all_logs = rcrrakeQuota.objects(pending_filters).order_by("-year", "-month")
            except Exception:
            ...
            paginated_data = pending_filtered_data[offset:offset + page_size]
            ...
            result.update({
                "labels": list(paginated_data[0].keys()) if paginated_data else [],
                "datasets": dataList,
            })
    ...
    def coal_journey_rcr_rake_quota_completed(self, currentPage, perPage, start_timestamp, end_timestamp):
        try:
            completed_filters = Q()
            result = {
                "labels": [],
            ...
            try:
                all_logs = rcrrakeQuota.objects(completed_filters).order_by("-year", "-month")
            except Exception:
                raise HTTPException(status_code=500, detail="Error processing rcrrakeQuota")
    ...
    def coal_journey_rcr_rake_quota_update(self, data):
        try:
            payload = data.dict()
            try:
...
coal_jouney_rcr_rake_quota = RcrRakeQuota()